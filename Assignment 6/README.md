...
> ⚠️ This notebook fetches **live data directly from the Open-Meteo API** at runtime — there is no dataset file to download. If the live request ever fails, it automatically falls back to a locally bundled sample (`sample_weather_response.csv`, included in this repo) with the same structure, purely so the notebook can still run end-to-end.
